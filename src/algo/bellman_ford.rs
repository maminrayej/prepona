...
impl<W: Clone + Any + Zero + Ord + std::fmt::Debug> BellmanFord<W> {
    ...
    pub fn execute<G, E: Edge<W>>(mut self, graph: &G, src_id: usize) -> Result<HashMap<(usize, usize), Magnitude<W>>, String>
    where
        G: provide::Vertices + provide::Edges<W, E>
    {
        ...
        let src_virt_id = id_map.get_real_to_virt(src_id).expect(&format!("{} is not valid", src_id));

        self.distance[src_virt_id] = W::zero().into();

        let edges = graph.edges(true);
