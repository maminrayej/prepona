...
#[derive(PartialEq, Eq)]
struct VertexScoreHue(RealID, Magnitude<usize>, Magnitude<usize>);

impl PartialOrd for VertexScoreHue {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        self.2.partial_cmp(&other.2)
    }
}

impl Ord for VertexScoreHue {
    ...
    H: Fn(usize) -> usize,
{
    ...
    heap.push(VertexScoreHue(
        src_rid,
        0.into(),
        estimate_of(src_rid.inner()).into(),
    ));

    while let Some(VertexScoreHue(u_rid, u_cost, _)) = heap.pop() {
        ...
        for eid in graph.outgoing_edges(u_rid.inner()) {
            ...
            if cost < current_cost {
                ...
                // FIXME: storing eid is not corrent. because parent of v can change. it may be
                // wrong in dijkstra too.
                visited_edges.insert(eid);
                heap.push(VertexScoreHue(
                    v_rid,
                    cost,
                    cost + estimate_of(v_rid.inner()).into(),
                ));
            }
        }
