...
pub struct BellmanFordSPT<'a, S> {
    pub(super) view: View<'a, S, NodeSetSelector<S>, EdgeSetSelector<S>>,
    pub(super) cost: HashMap<NodeID, isize>,
}

impl<'a, S> BellmanFordSPT<'a, S> {
    pub fn cost_of(&self, node: NodeID) -> isize {
        self.cost[&node]
    }
}
...
where
    S: Node,
{
    pub fn cost_of_checked(&self, node: NodeID) -> Result<isize, Error> {
...
impl<'a, S> Storage for BellmanFordSPT<'a, S>
where
    S: Storage,
{
    type Dir = S::Dir;
...
impl<'b, S> Node for BellmanFordSPT<'b, S>
where
    S: Node,
{
...
impl<'b, S> Edge for BellmanFordSPT<'b, S>
where
    S: Edge,
{
